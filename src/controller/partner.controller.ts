...
import Notification from '../models/notification.model';
  ...
  const getPartnerDetails = async (req: Request, res: Response): Promise<void> => {
    try {
      const mcpId = req.user?._id;
  ...
  const deletePartner = async (req: Request, res: Response): Promise<void> => {
    const session = await mongoose.startSession();
    session.startTransaction();
...
const PartnerController = {getAllPartners,getPartnerDetails,updatePartner,deletePartner,addPartner}
